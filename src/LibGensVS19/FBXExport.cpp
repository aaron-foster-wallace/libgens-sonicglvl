...
namespace LibGens {
	...
	FbxMesh* FBX::addNodeV2(Model* model, HavokSkeletonCache* skeleton, list <LibGens::HavokAnimationCache*> animation_list, Matrix4 transform_matrix) {
		string model_name = "Dummy";
		...
		return addNamedNodeV2(model_name, model, skeleton, animation_list, transform_matrix);
	}
	FbxMesh* FBX::addNamedNodeV2(string name, Model* model, HavokSkeletonCache* skeleton, list <LibGens::HavokAnimationCache*> animation_list, Matrix4 transform_matrix) {
		FbxNode* lMeshNode = NULL;
		FbxMesh* lMesh = NULL;
		...
		if (skeleton) {
			...
			if (havok_skeleton) {
				...
				std::list<LibGens::HavokAnimationCache*>::iterator it;
				for (it = animation_list.begin(); it != animation_list.end(); ++it)
					//for (LibGens::HavokAnimationCache * animation : animation_list)
				{
					LibGens::HavokAnimationCache* animation = *it;
					hkaAnimationBinding* havok_animation_binding = animation->getAnimationBinding();
					hkaAnimation* havok_animation = animation->getAnimation();
					addHavokAnimationNamed(animation->getName(),skeleton_bones, havok_skeleton, havok_animation_binding, havok_animation);
				}
			}
		}
